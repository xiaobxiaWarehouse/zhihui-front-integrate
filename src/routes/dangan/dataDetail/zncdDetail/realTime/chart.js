...
class Chart extends React.Component {
  ...
  componentDidMount() {
    const { location, dispatch, dangan } = this.props;
    const { search } = location;
    const { shishiZnc: hrList } = dangan;
    const query = queryString.parse(search);
    timer = true;
    dispatch({
      type: 'dangan/updateShishiZncd',
      payload: {
        shijian: undefined,
        hrList: [],
        rrList: [],
  ...
  render() {
    const { dangan, location } = this.props;
    const { shishiZnc } = dangan;
