...
const modal = (props) => {
  const {
    ...
    item = {},
    form: {
      getFieldDecorator, getFieldsValue, validateFields, setFieldsValue,
    },
    onOk,
    prev,
    onChange,
  ...
  const search = () => {
    ...
    validateFields((errors) => {
      ...
      const {
        signType, kaishiSj, jieshuSj, zuijin,
      } = fields;
  ...
  const onPage = (type) => {
    let fields = getFieldsValue();
    const {
      kaishiSj, jieshuSj, signType, zuijin,
    } = fields;
    if (kaishiSj || jieshuSj) {
      if (!kaishiSj || !jieshuSj) {
        message.error('必须选择开始时间与结束时间');
      ...
      <Form>
        <div style={{ textAlign: 'center' }}>
          <Row>
            ...
            <Col span={6}>
              <FormItem {...formItemLayout}>
                {getFieldDecorator('zuijin', {})(<Select
                  onChange={(val) => {
                      onChangeZuijin(val);
                    }}
                  allowClear
