...
export default {
  ...
  effects: {
    * checkLogin({payload}, {call, put, select}) {
      ...
      if (res.success) {
        let {result} = res;
        const {profile, permissions: permission} = result;
        const logoutMenu = {
          id: -1,
          code: 'sys:user:url:logout',
          description: null,
          name: '退出',
          path: '/logout',
          type: 1,
        ...
        if (result.login && profile.zhuangtai === 1) {
          ...
          if (config.openPages && config.openPages.indexOf(locationPathname) !== -1) {
    ...
    * updatePassword({ payload: obj, callback }, { call, put }) {
      ...
      if (res.success) {
        ...
        yield put({ type: 'hideUpdatePasswordModal'});
        yield put(routerRedux.push('/'));
      }
    },
  ...
  reducers: {
    ...
    changeCurrScrollTop(state, {payload}) {
      return {
        ...state,
        currScrollTop: payload,
      };
    },
