...
const {cityList, getCityName} = mapper;
// 连字符转驼峰
...
const filterMenu = (permission, menuList) => {
  ...
  menuList.forEach((item) => {
    let state = item.code.some((c) => {
      return codeList.indexOf(c) > -1 || c === 'sys:dashboard:dir:manage';
    });
    if (state) {
      newMenu = {
        id: item.id,
      ...
      newMenu.children = filterMenu(permission, item.children);
...
const getObjItem = (obj, list, fatherList) => {
  list.forEach((item) => {
    if (item._type === 'radio' || item._type === 'checkbox') {
      obj[item._id] = {
        cap: item._cap || (fatherList && fatherList._cap),
        children: item.option.map((o) => {
          return {
            value: o._value,
            content: o.__text,
          };
    ...
    } else if (item._type === 'mixed' || item._type === 'list' || item._type === 'table') {
      getObjItem(obj, item.subitem, item);
    } else {
      obj[item._id] = {
        cap: item._cap || (fatherList && fatherList._cap),
      };
...
const getXmlData = (data) => {
  let obj = {};
  try {
    const {document: {section}} = data;
    section.forEach((item) => {
      if (item && item.item && item.item.constructor === Array) {
        getObjItem(obj, item.item, item);
      } else if (item && item.item && item.item.subitem.constructor === Array) {
        getObjItem(obj, item.item.subitem, item.item);
      }
    });
    return obj;
  } catch (e) {
