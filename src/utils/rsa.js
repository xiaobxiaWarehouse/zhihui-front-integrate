...
function encrypt(text, key, isPub) {
  // Encrypt with key...
  const encryptKeyPem = isPub
    ? pki.publicKeyBodyToPem(key)
    : pki.privateKeyBodyToPem(key);
  const encryptKey = isPub
    ? pki.publicKeyFromPem(encryptKeyPem)
    : pki.privateKeyPKCS1ToPKCS8(pki.privateKeyFromPem(encryptKeyPem));
...
function decrypt(text, key, isPub) {
  // Decrypt with key...
  const decryptKeyPem = isPub
    ? pki.publicKeyBodyToPem(key)
    : pki.privateKeyBodyToPem(key);
  const decryptKey = isPub
    ? pki.publicKeyFromPem(decryptKeyPem)
    : pki.privateKeyPKCS1ToPKCS8(pki.privateKeyFromPem(decryptKeyPem));
